...
void logNetworkFile(struct Network*nw) {
    ...
    for (int i = 0; i < NUM_INPUTS; i++) {
        ...
        // DONT YOU DARE UNCOMMENT THIS, IT WILL SEGFAULT BECAUSE THE WEIGHTS FOR THE INPUT LAYER ARE NOT ALLOCATED(its a NULL pointer)
        //for (int j = 0; j < NUM_INPUTS; j++) {
        //    fprintf(file, "i = %d, j = %d\n", i, j);
        //    fprintf(file, "      input[%d]: %f\n", j, nw->inputs_layer[i].weights[j]);
        //}
    }
